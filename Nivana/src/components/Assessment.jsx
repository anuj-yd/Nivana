...
//     // ✅ FORCE NAVIGATION IF STUCK (Safety Net)
//     const safetyTimeout = setTimeout(() => {
//         console.warn("⚠️ API Timeout: Forcing navigation to dashboard.");
//         setSubmitting(false);
//         navigate("/dashboard");
...
//     try {
//       const data = await apiService.submitAssessment({ age: Number(age), answers: payloadAnswers });
//       clearTimeout(safetyTimeout); // API responded, cancel timeout
...
//         // ✅ IMPORTANT: Wait briefly then navigate
//         setTimeout(() => {
...
//       // ✅ Show LLM result modal if needed (only if staying on page)
//       if (analysis && (analysis.guidanceText || analysis.guidance)) {
//          setLlmResult({ text: analysis.guidanceText || analysis.guidance, severity: analysis.severity, risks: analysis.risks || [] });
//       }
...
//       if (err.response?.status === 401) {
//         setToast({ type: 'error', text: 'Unauthorized — please log in.' });
//         setTimeout(() => navigate('/login', { state: { next: '/assessments' } }), 1500);
//       } else {
//         setToast({ type: "error", text: err.response?.data?.msg || "Submission failed. Please try again." });
//       }
//       setSubmitting(false); // Stop loader on error
//     } 
//     // `finally` block removed because setSubmitting(false) is now handled explicitly in success/error blocks for better timing control
//   };
